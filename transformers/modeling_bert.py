...
class BertForOffensiveClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classfication_classifier = nn.Linear(config.hidden_size, self.config.num_labels)
        self.regression_classifier = nn.Linear(config.hidden_size, 1)
        self.init_weights()
    ...
    Examples::
        ...
        regression_logits = self.regression_classifier(pooled_output)
        outputs = (classification_logits, regression_logits) + outputs[2:]  # add hidden states and attention if they are here

        if classification_labels is not None and regression_labels is not None:
            regression_loss_fct = MSELoss()
            regression_loss = regression_loss_fct(regression_logits.view(-1), regression_labels.view(-1))
